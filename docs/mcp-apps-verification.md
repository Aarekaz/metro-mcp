...
- in normal scenarios, exact empty sequences for console/page errors, AppBridge errors, unexpected external requests or WebSockets, network-constructor calls, storage or cookie access, privileged-browser API access, unexpected protocol methods, protocol violations, and requests still awaiting responses;
- adversarial probes for every recorded surface, including caught opaque-origin access before an iframe remount, hybrid marker/JSON-RPC traffic, Apps-invalid method parameters, unsolicited/duplicate/mismatched/missing responses, malformed JSON-RPC, and the installed SDK's safe-integer, null, unsafe-integer, and own-`undefined` optional-property boundaries. Acknowledgements accept only the exact new ordered observations; mixed expected-plus-unexpected probes demonstrate that unrelated effects and AppBridge errors remain failures.
