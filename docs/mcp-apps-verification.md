...
`bun run test:apps` starts a loopback-only Vite server at `127.0.0.1:4178`, opens `tests/apps/host.html`, and uses `@playwright/test` `1.62.1`. The host loads `/apps/transit-board.html` in an iframe with exactly `sandbox="allow-scripts"`; it does not grant `allow-same-origin` or a permission-policy `allow` attribute.
...
- zero console errors, page errors, unexpected external requests, storage writes, permission calls, or unexpected protocol effects.
